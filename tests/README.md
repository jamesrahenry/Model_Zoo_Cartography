...
| `test_state_trajectory.py` | `null_baseline/state_trajectory.py` | The F2 instrument itself (the chain internals are in `test_chain_state_keyed.py`): exact q anchors (identity → 1, rank-one → 1/w), rank collapse on random He nets, and the null-band + z-score protocol in miniature (fresh init reads in-band; rank-collapsed weights read far below band) |
| `test_wild_census.py` | `census/wild_census.py` | The §3.8 entry-shuffle null: iid matrices read null vs their own shuffle, planted spikes are counted and destroyed by shuffling, transpose invariance (the documented pythia orientation artifact), and the F7 `depleted` regime flag |
| `test_refit_trained.py` | `null_baseline/refit_trained.py` | The §4.7 population protocol as corrected 2026-08-18: fit/val/transfer pairwise disjoint in every regime, and the documented facts (val = the two C=10 probe families that gate polish; converged transfer = untouched C=50) pinned so role changes must update the paper |
| `test_feature_tracker.py` | `census/feature_tracker.py` | The §3.7 negative result's instrument side: stable directions track as persistent features with cos ≈ 1 chains, an orthogonal mid-network rotation breaks tracking exactly at the boundary (raw tracking SHOULD fail across coordinate changes), low-variance PCs are filtered |
