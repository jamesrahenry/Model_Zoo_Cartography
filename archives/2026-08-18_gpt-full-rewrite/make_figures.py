...
def fig3():
    ...
    fig.suptitle("Activation measurements under noise and task input",
                 fontsize=11, color=INK, x=0.02, ha="left")
    fig.tight_layout(rect=(0, 0, 1, 0.94))
...
def fig4_rotation():
    ...
    a.annotate("raw overlap remains\nnear the reference baseline",
    ...
    fig.suptitle("Raw and Procrustes-aligned activation eigenspaces "
...
def fig5_wall():
    ...
    b.set_title("estimated C50 across width and budget", fontsize=10, color=INK,
                loc="left")
    style(b)

    fig.suptitle("Class-count convergence transition across width and budget",
                 fontsize=11, color=INK, x=0.02, ha="left")
    fig.tight_layout(rect=(0, 0, 1, 0.94))
...
def fig7_refit():
    ...
    ax.set_title("Population-fitted correction on trained weights",
...
def fig6_bulk():
    ...
    fig.suptitle("Bulk scale changes with little validation-accuracy change",
                 fontsize=11, color=INK, x=0.02, ha="left")
